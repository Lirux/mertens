...
description: >-
  Stage only files changed by Codex during the current session, create an atomic
  Conventional Commit, and push it to the matching branch on origin. Use when
  the user asks to commit and push, publish changes, push after committing, or
  create a semantic or conventional commit and send it to origin. Preserve
  pre-existing user changes and never force-push implicitly.
---
...
6. Read the current branch with `git branch --show-current` and stop if HEAD is
   detached.
7. Verify that the `origin` remote exists with `git remote get-url origin`.
...
diff contains unrelated concerns, do not commit or push. Identify the concerns
and recommend how to split them, but do not alter already-staged user work
...
Never use `--force`, `--force-with-lease`, change the remote, or push a different
branch unless the user explicitly requests that exact action. If the push
fails, explain the error and stop; do not retry with a more destructive command.
The local commit remains created even when the push fails.
...
Report the commit hash and subject plus `origin/<current-branch>`. Mention any
